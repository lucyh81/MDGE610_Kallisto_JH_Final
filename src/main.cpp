...
void ParseOptionsBus(int argc, char **argv, ProgramOptions &opt) {
  ...
  if (opt.technology.find('%') !=
      std::string::npos) {  // Process technologystd::strings of format -x bc:umi:cdna%strand%parity
...
bool ParseTechnology(const std::string &techstr, BUSOptions &busopt,
  ...
  if (i1 == std::string::npos) {
    errorList.push_back("Error: technologystd::string must contain two colons (:), none found: \"" +
                        techstr + "\"");
    return false;
  }
  auto i2 = techstr.find(':', i1 + 1);
  if (i2 == std::string::npos) {
    errorList.push_back("Error: technologystd::string must contain two colons (:), only one found: \"" +
                        techstr + "\"");
    return false;
  }
  auto ip = techstr.find(':', i2 + 1);
  if (ip != std::string::npos) {
    errorList.push_back("Error: technologystd::string must contain two colons (:), three found: \"" +
                        techstr + "\"");
    return false;
  }
...
// check bam options
  ...
  if (opt.technology.empty()) {  // kallisto pseudo
    ...
    if (opt.technology == "10XV2") {
      busopt.nfiles = 2;
      busopt.seq.push_back(BUSOptionSubstr(1, 0, 0));    // second file, entirestd::string
      ...
      std::vector<BUSOptionSubstr> bcValues;
      std::vector<std::string> errorList;
      ...
      if (valid) {
        ...
        for (int j = 0; j < errorList.size(); j++) {
          std::cerr << errorList[j] << std::endl;
        }
        std::cerr << "Unable to create technology: " << opt.technology << std::endl;
        ret = false;
      }
    }
    if (opt.bam) {
   ...
   std::string cmd(argv[1]);
    if (cmd == "version") {
      ...
      if (!CheckOptionsEM(opt)) {
        ...
        em.run(3000, 3000, true, opt.bias);
